...
2. **提取**：如果存在，从错误描述中提取出缺失的字段名或指标名，并区分类型
...
**错误信息：**
```
- 错误：HQL 中引用了 order_quantity 字段，但该字段不在当前上下文的可用字段列表中
- 错误：指标"月销售量"未在已召回的指标列表中，导致无法按定义口径计算
```
...
**错误信息：**
```
- 错误：时间范围偏差，用户要求近12个月，HQL 使用了当年1月至今
- 错误：COUNT(user_id) 未去重，应使用 COUNT(DISTINCT user_id)
```
